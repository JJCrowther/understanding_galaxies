...
i=0 #scale factor for multiplication
scale_factor_data={}
averages={}
#cut_threshold = 0.8
...
for entry in range(len(split_by_redshift)):
    averages[entry] = frf.average_maker(split_by_redshift[entry])

x_data = np.arange(0, 0.91, 0.01)

y_data = np.zeros((0, 3))
for index in averages:
    y_data = np.vstack((y_data, averages[index][0:3].astype(float)))

frf.error_bar_smoothness_3(x_data, y_data[:, 0:1], y_data[:, 1:2], y_data[:, 2:3], save_name='smoothness_comparison_graph_redshift.png', title='Galaxy Morphology with Redshift', xlabel='Redshift', ylabel='Average Probability', ylimits=[0, 1])
