...
if __name__ == '__main__':
    ...
    test_sample_names = full_data_array_first_cut[73:84, 0] 
    ...
    for test_name in test_sample_names:
        ...
        prediction_average = np.mean(prediction_gp)
        sigma_average = np.mean(np.sqrt(pred_sigma_list))
        plt.errorbar(pred_z, prediction_average, sigma_average, marker='v', label='Prediction\nPrediction value: {0:.3f}\nStandard Deviation: {1:.3f}'.format(prediction_average, sigma_average))
        plt.xlabel('Redshift')
        ...
        if (len(peaks) == 1):  
            ...
            plt.savefig('{0}_kernal_prediction_smooth_gpinterpolation.png'.format(test_name))
            plt.close()
