...
if __name__ == '__main__':
    running_chi_squared=[]
    for delta_p in np.arange(0.05, 0.5, 0.05):
        ...
        for file_name in file_name_list:
            ...
            i+=1 
        print('Data for delta_p={} read, moving onto predictions'.format(delta_p))
        ...
        print('Finished {0:.3f} pass'.format(delta_p))

    plt.figure(figsize=(10,6))
    plt.suptitle('Optimising Reduced $\chi^2$ Parameters (Probability ranges)', fontsize=18)
    plt.errorbar(np.arange(0.2, 2.1, 0.2), running_chi_squared, marker ='x', alpha=1, label='Reduced Chi-Squared')
    ...
    plt.savefig('optimising_reduced_chi_squared_prob_extended.png'.format(name), dpi=200)
    plt.close()
