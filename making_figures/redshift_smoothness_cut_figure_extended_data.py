...
for proportions in proportions_by_redshift_by_cut:
    ...
    frf.error_bar_smoothness_3(x_data_list[i], y_data[:, 0:1], y_data[:, 1:2], y_data[:, 2:3], save_name=figure_save_names[i], title='Galaxy Morphology with Redshift for Magnitudes {0}'.format(figure_mag_lims[i]), xlabel='Redshift', ylabel='Proportion of expected predictions', ylimits=[0, 1], xlimits=[0.02, 0.25])
    i+=1
