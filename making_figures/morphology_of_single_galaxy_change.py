...
if __name__ == '__main__':
    ...
    test_sample_names = full_data_array_first_cut[76:79, 0] 
    ...
    for test_name in test_sample_names:
        ...
        test_galaxy = test_sample[test_sample[0] == test_name].reset_index(drop = 'true')
        
        test_gal_correction_z = []
        test_gal_correction_prob = []
        test_gal_correction_sd = []
        
        for idx in range(test_galaxy[4].astype(float).idxmax()):
            
            gal_max_z = test_galaxy.loc[[idx]]
            ...
            test_gal_correction_z.append(test_z)
            test_gal_correction_prob.append(weighted_mean)
            test_gal_correction_sd.append(weighted_std)
            
        test_gal_x_data = test_galaxy[4].to_numpy().astype(float)
        test_gal_y_data = test_galaxy[1].to_numpy().astype(float)
        
        plt.figure(figsize=(10,6))
        plt.suptitle('Comparison of {0} de-biasing'.format(test_name), fontsize=18)
        
        plt.subplot(111)
        
        plt.errorbar(test_gal_correction_z, test_gal_correction_prob, test_gal_correction_sd, ls = 'dashed', alpha = 1, marker ='x', color = 'blue', label = 'De-baised evolution')
        plt.errorbar(test_gal_x_data, test_gal_y_data, marker = 'x', alpha=1, color='red', label = 'Test Gal {0}'.format(test_name))
        ...
        plt.savefig('a_{0}.png'.format(test_name), dpi=200)
        plt.close()
