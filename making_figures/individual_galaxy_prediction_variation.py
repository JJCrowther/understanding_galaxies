...
for file_name in file_name_list:
    ...
    merged_dataframe['redshift']=merged_dataframe['redshift'].mul(i) #Multiplies the redshift by the scalefactor (should happen before clip? But doesn't make difference here since clip is so small)
...
for line in range(len(full_data_array_probs[1])):
    ...
    plt.errorbar(x_data_list, y_data_list, y_err, marker ='x', alpha=0.5)
...
plt.savefig('Individual_galaxy_predictions_smooth.png', dpi=200)
plt.close()
